...
async function createPage(data, page){
	...
	var title = data.name+" Lodge No. "+data.lodge_num;
	var url = "https://thelostjewel.org/index.php?title="+encodeURIComponent(title.replace(/\s+/g, '_').replace(/\./g, ''))+"&action=edit";
	var content = `{{Lodge
|name=${data.name}
|number=${data.lodge_num}
|status=Active
|juris=Grand Lodge of Georgia
|founded=${year}
|location=${data.city}, Georgia
|meeting=${data.meeting_times}
}}

'''${data.name} No. ${data.lodge_num}''' is located in ${data.city}, Georgia. It received its charter on ${data.charter_date} from the Grand Lodge of Georgia.

[[Category:Lodges]]
[[Category:Grand Lodge of Georgia]]`;
	var summary = `${data.name} Lodge no. ${data.lodge_num} in ${data.city}, Georgia.`;
